...
import { Plus, Trash2, Edit2, LogOut, Eye, Users, Save, RefreshCw, Search, AlertTriangle, X, Check, DollarSign, ShieldAlert, TestTube, Unlock, Ban, Calendar, User as UserIcon, Sparkles, Megaphone, Clock, ArrowDownUp, Filter } from 'lucide-react';
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <header className="flex flex-col gap-6 bg-white p-6 rounded-3xl shadow-sm">
           <div className="flex flex-col md:flex-row justify-between items-center gap-4">
              <div><h1 className="text-3xl font-extrabold text-gray-900 tracking-tight">Painel Admin</h1><p className="text-gray-500 font-medium">Gestão EuDorama</p></div>
              <button onClick={onLogout} className="flex items-center text-red-600 font-bold hover:bg-red-50 px-4 py-2 rounded-xl transition-colors"><LogOut className="w-5 h-5 mr-2" /> Sair</button>
           </div>
           {/* STATS (Keep existing) */}
           <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
               <div className="bg-blue-50 p-4 rounded-2xl border border-blue-100"><div className="flex items-center text-blue-600 mb-1"><Users className="w-5 h-5 mr-2" /><span className="font-bold text-sm">Clientes</span></div><span className="text-2xl font-black text-blue-900">{stats.totalClients}</span></div>
               <div className="bg-green-50 p-4 rounded-2xl border border-green-100"><div className="flex items-center text-green-600 mb-1"><DollarSign className="w-5 h-5 mr-2" /><span className="font-bold text-sm">MRR (Est.)</span></div><span className="text-2xl font-black text-green-900">R$ {stats.totalRevenue}</span></div>
               <div className="bg-yellow-50 p-4 rounded-2xl border border-yellow-100"><div className="flex items-center text-yellow-600 mb-1"><Calendar className="w-5 h-5 mr-2" /><span className="font-bold text-sm">A Vencer</span></div><span className="text-2xl font-black text-yellow-900">{stats.expiringSoon}</span></div>
               <div className="bg-red-50 p-4 rounded-2xl border border-red-100"><div className="flex items-center text-red-600 mb-1"><AlertTriangle className="w-5 h-5 mr-2" /><span className="font-bold text-sm">Inadimplentes</span></div><span className="text-2xl font-black text-red-900">{stats.debtors}</span></div>
           </div>
        </header>
        ...
        {/* ... KEEP OTHER TABS (Clients, Test, Danger, etc.) IDENTICAL ... */}
        {activeTab === 'clients' && (
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                <div className="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                    ...
                    <div className="flex gap-2"><button onClick={handleCreateDemo} disabled={loading} className="bg-purple-600 text-white px-6 py-3 rounded-xl font-bold flex items-center hover:bg-purple-700 shadow-lg shadow-purple-100 transition-transform active:scale-95">{loading ? <RefreshCw className="w-5 h-5 animate-spin" /> : <><Sparkles className="w-5 h-5 mr-2" /> Gerar Demo</>}</button><button onClick={() => handleOpenClientModal()} className="bg-green-600 text-white px-6 py-3 rounded-xl font-bold flex items-center hover:bg-green-700 shadow-lg shadow-green-100 transition-transform active:scale-95"><Plus className="w-5 h-5 mr-2" /> Novo Cliente</button></div>
                </div>
                <div className="overflow-x-auto rounded-xl border border-gray-100 shadow-inner">
                    <table className="w-full text-sm text-left min-w-[600px]">
                        <thead className="bg-gray-50 text-gray-500 font-bold uppercase tracking-wider text-xs"><tr><th className="p-4">Cliente</th><th className="p-4">Serviços</th><th className="p-4">Status / Expiração</th><th className="p-4 text-right">Opções</th></tr></thead>
                        <tbody className="divide-y divide-gray-100 bg-white">
                            {filteredClients.map(client => (
                                <tr key={client.id} className={`hover:bg-gray-50 transition-colors ${client.is_debtor ? 'bg-red-50' : ''}`}>
                                    <td className="p-4"><div className="flex flex-col"><span className="font-bold text-gray-900 font-mono text-base">{client.phone_number}</span>{client.client_name && <span className="text-gray-600 text-xs font-bold">{client.client_name}</span>}<span className="text-gray-400 text-xs">Senha: {client.client_password || 'Sem senha'}</span></div></td>
                                    <td className="p-4"><div className="flex flex-wrap gap-1">{(Array.isArray(client.subscriptions) ? client.subscriptions : []).map(s => <span key={s} className="bg-gray-100 text-gray-600 px-2 py-0.5 rounded text-[10px] font-bold uppercase">{s.split('|')[0]}</span>)}</div></td>
                                    <td className="p-4"><div className="flex flex-col"><span className="font-medium text-gray-800">{new Date(client.purchase_date).toLocaleDateString('pt-BR')}</span><span className="text-[10px] text-gray-500 font-bold uppercase tracking-wide">Plano: {client.duration_months} Mês(es)</span><div className="flex flex-wrap gap-1 mt-1">{client.is_debtor && <span className="text-red-600 font-bold text-xs bg-red-100 px-2 rounded">Bloqueado</span>}{client.override_expiration && <span className="text-yellow-600 font-bold text-xs bg-yellow-100 px-2 rounded">Liberado</span>}</div></div></td>
                                    <td className="p-4 text-right"><div className="flex justify-end gap-2"><button onClick={() => handleQuickToggleOverride(client)} className={`p-2 rounded-lg ${client.override_expiration ? 'bg-yellow-100 text-yellow-700' : 'text-gray-400'}`} title="Liberar Acesso Vencido"><Unlock className="w-4 h-4" /></button><button onClick={() => handleOpenClientModal(client)} className="text-blue-600 p-2"><Edit2 className="w-4 h-4" /></button><button onClick={() => setDeleteModal({open: true, type: 'client', id: client.id})} className="text-red-600 p-2"><Trash2 className="w-4 h-4" /></button></div></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
